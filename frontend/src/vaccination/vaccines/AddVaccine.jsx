...
import { createVaccine } from "../../store/vaccination/vaccineSlice";
...
export default function AddVaccine() {
  ...
  const { loading, error } = useSelector((state) => state.vaccine);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(createVaccine(vaccineData)).unwrap();
      navigate("/vaccines");
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-800 shadow-md rounded-md mt-10">
      <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Add New Vaccine</h2>
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex justify-between">
          ...
          >
            {isSubmitting || loading ? "Loading..." : "Add"}
          </button>
