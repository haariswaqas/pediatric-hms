...
import { adminItems as adminGroups } from './utils/groupedButtons';

export default function AdminSidebar({ collapsed }) {
  const { pathname } = useLocation();
  const [openGroups, setOpenGroups] = useState(() =>
    adminGroups.reduce((acc, item) => ({ ...acc, [item.label]: true }), {})
  );
        ...
        {adminGroups.map(item => (
          <li key={item.label}>
            ...
            >
              ...
              {!collapsed && (
                openGroups[item.label] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />
              )}
            </button>
