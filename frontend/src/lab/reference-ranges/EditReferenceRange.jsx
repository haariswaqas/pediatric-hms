...
import { updateReferenceRange, fetchReferenceRangeById } from '../../store/lab/referenceRangeSlice';
...
const EditReferenceRange = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { referenceRangeId } = useParams();
  const { selectedReferenceRange: referenceRange, loading, error } = useSelector((state) => state.referenceRange);
...
useEffect(() => {
 ...
 if (referenceRangeId) dispatch(fetchReferenceRangeById(referenceRangeId))
}, [dispatch, referenceRangeId])

useEffect(() => {
  if (referenceRange) {
    setFormData({
      lab_test: referenceRange.lab_test || "",
      parameter_name: referenceRange.parameter_name || "",
      min_age_months: referenceRange.min_age_months || 0,
      max_age_months: referenceRange.max_age_months || 216,
      gender: referenceRange.gender || "ALL",
      min_value: referenceRange.min_value || "",
      max_value: referenceRange.max_value || "",
      unit: referenceRange.unit || "",
      textual_reference: referenceRange.textual_reference || "",
      notes: referenceRange.notes || ""
    })
  }
}, [referenceRange])
...
const handleChange = (e) => {
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      min_age_months: formData.min_age_months || 0,
      max_age_months: formData.max_age_months || 216,
      gender: formData.gender || 'ALL',
      min_value: formData.min_value || '',
      max_value: formData.max_value || '',
      unit: formData.unit || '',
      textual_reference: formData.textual_reference || '',
      notes: formData.notes || ''
    };
    
    // Log the payload to verify it contains required fields
    console.log("Submitting reference range update with payload:", payload);
    
    dispatch(updateReferenceRange({ referenceRangeId, updatedData: payload })).then((res) => {
      if (!res.error) {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-center py-12">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-600 dark:text-gray-300">Loading reference range data...</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                Update parameter ranges by age and gender
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  ...
                  {labTests.map((lt) => (
                    <option key={lt.id} value={lt.code}>{lt.name} ({lt.code})</option>
                  ))}
                </select>
              </div>
              <div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows={3}
                  className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-400 placeholder-gray-500 dark:placeholder-gray-400"
                />
              </div>
            </div>
          </div>
...
export default EditReferenceRange;